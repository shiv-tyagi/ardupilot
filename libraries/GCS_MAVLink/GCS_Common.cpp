...
void GCS_MAVLINK::handle_global_position_int(const mavlink_message_t &msg)
{
    ...
    AP_LocationDB_Item item(key, AP_HAL::millis(), pos, vel, Vector3f(0, 0, 0), packet.hdg, 0, populated_fields);
    db->add_item(item);
}
...
void GCS_MAVLINK::handle_follow_target(const mavlink_message_t &msg)
{
    ...
    AP_LocationDB_Item item(key, AP_HAL::millis(), pos, vel, Vector3f(0, 0, 0), heading, 0, populated_fields);
    db->add_item(item);
}
#endif
