...
{   
    ...
    if (item.is_field_populated(AP_LocationDB_Item::DataField::POS)) {
    ...
    if (item.is_field_populated(AP_LocationDB_Item::DataField::VEL)) {
        ret.fields[field_index++] = item._vel.x;
        ret.fields[field_index++] = item._vel.y;
        ret.fields[field_index++] = item._vel.z;
    }

    if (item.is_field_populated(AP_LocationDB_Item::DataField::ACC)) {
        ret.fields[field_index++] = item._acc.x;
        ret.fields[field_index++] = item._acc.y;
        ret.fields[field_index++] = item._acc.z;
    }

    if (item.is_field_populated(AP_LocationDB_Item::DataField::HEADING)) {
        ret.fields[field_index++] = item._heading;
    }

    if (item.is_field_populated(AP_LocationDB_Item::DataField::RADIUS)) {
